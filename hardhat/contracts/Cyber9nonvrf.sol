...
contract Cyber9nonvrf is ERC721Enumerable, Ownable, RNG, VRFConsumerBase {
    ...
    function reserveMint() internal {
        for(uint256 i = 1; i<startFrom+1; i++) { 
            if(i == 1) {
                characters[i].setStats(1, 200, 31, 16, 16, 21, 21, 400000);  
                characterMaxHp[i] = 200;
            }else if(i == 2) {
                characters[i].setStats(1, 150, 21, 31, 16, 21, 21, 400000);
    ...
    function freeMint(uint256 _badgeId) external {
     ...
     function mintLogic(uint256 _amount) private {
        ...
        for(uint256 i=0; i<_amount;i++) {
            ...
            (_hp, _strength,_agility,_intelligence,_dexterity,_luck) = generateStats(_newId, _newId);
    ...
    function attackEnemy(uint256[] memory _tokensId) external {
        ...
        for(uint256 i = 0; i<_tokensId.length;i++) {
            ...
            // getRandomNumber();
            uint256 _dmg = roll(characters[_tokensId[i]].strength, 1, _tokensId[i]) + walletOfOwner(msg.sender).c1Bonus();
            bool isCrit = false;
            ...
            uint256 _enemyDmg = uint256(roll(_enemyAtk, 2, _tokensId[i]));
